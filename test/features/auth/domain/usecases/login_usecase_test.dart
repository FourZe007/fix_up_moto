...
void main() {
  ...
  // The phone is passed through as typed; normalisation happens in the data layer.
  const tParams = LoginParams(phone: '081234567890', password: 'Password1');

  // ── Happy path ────────────────────────────────────────────────────────────

  test(
    'should return UserEntity from repository when login succeeds',
    () async {
      ...
      verify(
        () => mockRepository.login(tParams.phone, tParams.password),
      ).called(1);
  ...
  test('should return AuthFailure when credentials are invalid', () async {
    ...
    expect(result, const Left(AuthFailure('Invalid phone number or password')));
    verify(
      () => mockRepository.login(tParams.phone, tParams.password),
    ).called(1);
  });
